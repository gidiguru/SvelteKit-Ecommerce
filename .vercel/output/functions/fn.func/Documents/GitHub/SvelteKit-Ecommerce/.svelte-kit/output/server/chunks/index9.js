import { s as subscribe, h as compute_rest_props } from "./lifecycle.js";
import { c as create_ssr_component, s as spread, d as escape_object, v as validate_component, b as escape_attribute_value } from "./ssr.js";
import { F as setCtx, G as getCtx, H as getAttrs, a as cn, n as flyAndScale } from "./utils.js";
import { i as is_void } from "./names.js";
import { c as createDispatcher } from "./events.js";
import { b as buttonVariants } from "./index6.js";
import { f as fade } from "./index5.js";
...
const AlertDialogTitle = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let builder;
  let $$restProps = compute_rest_props($$props, ["level", "asChild", "id"]);
  let $title, $$unsubscribe_title;
  let { level = "h2" } = $$props;
  let { asChild = false } = $$props;
  let { id = void 0 } = $$props;
  const { elements: { title }, ids } = getCtx();
  ...
  return `${asChild ? `${slots.default ? slots.default({ builder, attrs }) : ``}` : `${((tag) => {
    return tag ? `<${level}${spread([escape_object(builder), escape_object($$restProps), escape_object(attrs)], {})}>${is_void(tag) ? "" : `${slots.default ? slots.default({ builder, attrs }) : ``}`}${is_void(tag) ? "" : `</${tag}>`}` : "";
  })(level)}`}`;
});
...
const AlertDialogOverlay = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$unsubscribe_overlay();
  $$unsubscribe_open();
  return `${asChild && $open ? `${slots.default ? slots.default({ builder, attrs }) : ``}` : `${transition && $open ? `<div${spread([escape_object(builder), escape_object($$restProps), escape_object(attrs)], {})}></div>` : `${inTransition && outTransition && $open ? `<div${spread([escape_object(builder), escape_object($$restProps)], {})}></div>` : `${inTransition && $open ? `<div${spread(
    [
      escape_object(builder),
      escape_object($$restProps),
      escape_object(attrs)
