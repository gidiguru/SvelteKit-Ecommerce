...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  while (selectedSizeIdx < data.product.productTypes.length && !data.product.productTypes[selectedSizeIdx].isAvailable)
    selectedSizeIdx++;
  let curIdx = 0;
  let scrollSection;
  if ($$props.data === void 0 && $$bindings.data && data !== void 0)
    $$bindings.data(data);
  $$result.css.add(css);
  return `${$$result.head += `<!-- HEAD_svelte-tzzowm_START -->${$$result.title = `<title>${escape(data.product.name)} | Synergetics Shop</title>`, ""}<meta name="description"${add_attribute("content", data.product.description, 0)}><!-- HEAD_svelte-tzzowm_END -->`, ""} <div class="grow flex flex-col sm:pt-10 w-full"> <div class="w-full flex flex-col justify-center items-center gap-y-2 overflow-hidden sm:hidden"> <div class="w-screen overflow-hidden relative"><div class="overflow-x-auto snap-x snap-mandatory scroll-smooth flex slides w-full svelte-1srnmqy"${add_attribute("this", scrollSection, 0)}>${each(data.product.images, (image) => {
  ...
  })}</div></div></div>  <div class="flex flex-col items-center gap-y-4 md:w-1/2 m-auto text-center px-3"><h2 class="font-jura text-5xl py-6 text-neutral-800">${escape(data.product.name)}</h2></div> <div class="h-full sm:mx-10 mx-4 sm:grid sm:grid-cols-4 flex flex-col"><div class="col-span-3 p-4 relative h-[85vh] hidden sm:flex">${each(data.product.images, (im, i) => {
